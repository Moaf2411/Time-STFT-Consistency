...
def compute_stft_librosa(signal, sr, window_size, hop_size, min_freq, max_freq, nfft):
    stft_matrix = librosa.stft(signal, n_fft=nfft, hop_length=hop_size, win_length=window_size)

    freqs = librosa.fft_frequencies(sr=sr, n_fft=nfft)
    mel_freqs = librosa.mel_frequencies(n_mels=len(freqs), fmin=min_freq, fmax=max_freq)

    min_bin = np.argmax(freqs >= min_freq)
    max_bin = np.argmax(freqs > max_freq)
    stft_matrix = stft_matrix[min_bin:max_bin, :]

    stft_db = librosa.amplitude_to_db(np.abs(stft_matrix), ref=np.max)

    stft_db_normalized = (stft_db - np.min(stft_db)) / (np.max(stft_db) - np.min(stft_db))

    return stft_db_normalized
...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode, target_dataset_size=64, subset=False):
        ...
        stft = np.load('D:\\stft'+str(self.len)+'.npy')
        self.x_data_f = stft
